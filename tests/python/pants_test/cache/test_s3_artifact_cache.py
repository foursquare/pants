...
class FakeExtractor(object):
  ...
  def decompress_tarball(self, tarfile_path, dest_dir):
    with tarfile.open(tarfile_path, mode='r:gz') as archive:
      archive.extractall(path=dest_dir.decode('utf-8'))
...
def s3_fixture():
  TarballArtifact.NATIVE_BINARY = FakeExtractor()
  m = mock_s3()
  m.start()
...
def setup_test_file(parent):
  with temporary_file(parent, binary_mode=False) as f:
    # Write the file.
